...
async function main() {
  ...
  const admin = await prisma.user.upsert({
    where: { email: "admin@smartos.com" },
    update: {},
    create: { email: "admin@smartos.com", name: "Admin User", password, role: "ADMIN" },
  });

  const manager = await prisma.user.upsert({
    where: { email: "manager@smartos.com" },
    update: {},
    create: { email: "manager@smartos.com", name: "Sarah Manager", password, role: "MANAGER" },
  });

  const user1 = await prisma.user.upsert({
    where: { email: "alice@smartos.com" },
    update: {},
    create: { email: "alice@smartos.com", name: "Alice Developer", password, role: "USER" },
  });

  const user2 = await prisma.user.upsert({
    where: { email: "bob@smartos.com" },
    update: {},
    create: { email: "bob@smartos.com", name: "Bob Designer", password, role: "USER" },
  });
  ...
  console.log("  User:    alice@smartos.com");
  console.log("  User:    bob@smartos.com");
}
